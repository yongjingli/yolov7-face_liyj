...
# --weights /home/pxn-lyj/Egolee/programs/yolov7-face/local_files/yolov7-tiny.pt
# --batch-size 4
# --img-size 768 1280
# --device 0
# --grid 同时到处包含anchor解析的结果, 当设置这个选项的时候，需要将--device设置为cpu 和 --use_fp16 去除, 可能跟推理的一些op有关
# --use_fp16 使用fp16         # 将model和img输入这只为fp16,如果模型加载在fp32的模式下输入为fp16将会报错
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_fp16', action='store_true', help='use fp 16')
    ...
    try:
        ...
        with torch.no_grad():
            out = model(img)
        # model_torch2trt = torch2trt(model, [img],
        #                             min_shapes=[(1, 3, h, w)],
        #                             max_shapes=[(n, 3, h, w)],
        #                             max_batch_size=n)

        model_torch2trt = torch2trt(model, [img], use_onnx=True, fp16_mode=opt.use_fp16)

        # export torch2trt
        f = opt.weights.replace(".pt", "_trt_batch_{}.pth".format(n))   # filename
        torch.save(model_torch2trt.state_dict(), f)
        print("Done export torch2trt")

        model_torch2tr2 = TRTModule()
        model_torch2tr2.load_state_dict(torch.load(f, map_location=opt.device))
        model_torch2tr2.eval()
        out = model_torch2tr2(img)
